...
function Clip({ clip, track, pxPerSec, isSelected, onSelect, onTrim, onMove, onMoveToNewTrack, trackMuted, trackHidden, waveformSize, media }) {
  ...
  return (
    ...
    >
      ...
      {(clip.type === 'audio' || clip.type === 'video') && Array.isArray(realWaveform || waveform) && (realWaveform || waveform).length > 0 && (
        <div className="absolute inset-x-1.5 top-2 bottom-2 flex items-end gap-px pointer-events-none overflow-hidden">
          {(realWaveform || waveform).map((height, i) => (
            <div
              key={i}
              className={`flex-1 min-w-[1px] rounded-t transition-all ${
                realWaveform ? 'bg-white/50' : 'bg-white/30'
              }`}
